...
import {
  Monitor,
  MonitorCreatePumpFunMonitorParams,
  MonitorCreatePumpFunMonitorResponse,
  MonitorCreateUserFollowingMonitorParams,
  MonitorCreateUserFollowingMonitorResponse,
  MonitorCreateUserProfileMonitorParams,
  MonitorCreateUserProfileMonitorResponse,
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['SOCIAL_DATA_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['SOCIAL_DATA_LOG'] or 'warn' if it isn't set.
   */
...
export class SocialData {
   ...
   * API Client for interfacing with the Social Data API.
   *
   * @param {string | undefined} [opts.bearerToken=process.env['SOCIAL_DATA_BEARER_TOKEN'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['SOCIAL_DATA_BASE_URL'] ?? https://api.socialdata.tools] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('SOCIAL_DATA_BASE_URL'),
    bearerToken = readEnv('SOCIAL_DATA_BEARER_TOKEN'),
    ...opts
  }: ClientOptions = {}) {
    if (bearerToken === undefined) {
      throw new Errors.SocialDataError(
        "The SOCIAL_DATA_BEARER_TOKEN environment variable is missing or empty; either provide it, or instantiate the SocialData client with an bearerToken option, like new SocialData({ bearerToken: 'My Bearer Token' }).",
      );
    }
    ...
    const options: ClientOptions = {
      bearerToken,
      ...opts,
      baseURL: baseURL || `https://api.socialdata.tools`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? SocialData.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('SOCIAL_DATA_LOG'), "process.env['SOCIAL_DATA_LOG']", this) ??
      defaultLogLevel;
  ...
  static SocialData = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static SocialDataError = Errors.SocialDataError;
...
SocialData.Monitors = Monitors;
export declare namespace SocialData {
